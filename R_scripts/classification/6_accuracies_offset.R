## Calculate the median and the 1st, 3rd quartile of the accuracies after
## applying the offset
...
for (TISSUE_OF_INTEREST in c("breast", "colorectal", "ovarian"))
{
  ...
  idx_samples <- .vector(mode = "list", length = 2, n = SETS)
  ...
  for (m in 1:length(MU))
  {
    ...
    for (n in 1:length(NUM_PIXELS))
    {
      data_cv_env <- new.env()
      load(paste0(
        output_dir, "/conf_mat_sampling_MU_", MU[m], "_sample_", NUM_PIXELS[n],
        "pix.RData"
      ),
      envir = data_cv_env
      )
      ...
      {
        cat(paste0("Offset: ", MU[m], ", sample size: ", NUM_PIXELS[n], ", rep:", ir, "\n"))

        for (k in 1:9) # k = ncomps - 1
        {
          res_cv <- resultsClassif(data_cv_env$conf_mat_cv[[ir]][[k]])
    ...
    for (n in 1:length(NUM_PIXELS))
    {
      for (k in 1:9) # k = ncomps - 1
      {
        acc_cv_median[n, k] <- median(acc_cv[n, k, ])
